...
def analyze_domain(domain):
    ...
    try:
        ...
        if domain.endswith('.edu'):
            ...
            result['domain_info']['type'] = 'Commercial'
        elif domain.endswith('.net'):
            result['domain_info']['type'] = 'Network'
        elif domain.endswith('.ac.in'):
            result['domain_info']['type'] = 'Academic (India)'
        
        # Try to determine company name
        if domain.endswith('.ac.in'):
            result['domain_info']['company'] = 'Academic Institution'
        elif domain == 'gmail.com':
            result['domain_info']['company'] = 'Google'
        elif domain == 'yahoo.com':
            result['domain_info']['company'] = 'Yahoo'
        elif domain == 'hotmail.com' or domain == 'outlook.com':
            result['domain_info']['company'] = 'Microsoft'
...
def analyze_email_pattern(email):
    ...
    try:
        logger.info(f"Starting pattern analysis for email: {email}")
        username = email.split('@')[0]
        
        # Check for special characters
        has_special_chars = any(char in username for char in '._-')
        
        # Check for random-looking username
        is_random = (
            len(username) > 10 or
            any(char.isdigit() for char in username) or
            has_special_chars
        )
        
        # Check for common names
        common_names = ['john', 'jane', 'admin', 'user', 'test', 'info', 'support']
        has_common_name = any(name in username.lower() for name in common_names)
...
async def validate_email():
    try:
        ...
        validation_result = get_enhanced_validator().validate_email(email)
